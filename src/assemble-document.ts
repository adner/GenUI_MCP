...
//     imports. The agent's generated HTML uses these directly.
//   - Widens connect-src to include the same CDN hosts so chart.js etc. can
//     fetch sub-resources after their initial ESM bootstrap.
//
// WARNING: Keep this in rough sync when the OpenGenerativeUI design system
// changes substantively. There is no automated sync — re-fork manually.
...
:root {
  ...
  /* Aliases for variables the agent sometimes invents and references with
     fallbacks like var(--color-bg-surface, white). Provide them so the agent's
     CSS works in both themes instead of resolving to literal "white". */
  --color-bg-surface: var(--color-background-primary);
...
/* Host-driven dark theme. Mirrored from the prefers-color-scheme block below;
   set by BRIDGE_JS in response to a 'set-theme' postMessage from the parent
   so the iframe follows the MCP host's theme rather than the OS preference. */
...
// Theme messages from the parent View: { type: 'set-theme', theme: 'light'|'dark' }.
// We set data-theme on documentElement so :root[data-theme="dark"] CSS kicks in,
// and color-scheme so native widgets (scrollbars, form controls) follow suit.
...
function reportHeight() {
  ...
  // Measure across multiple candidates and take the max — robust to mixes of
  // flow layout, absolute positioning, and content that uses 100vh inside.
  // The clone-with-auto-height trick handles content that intrinsically wants
  // to fill the iframe viewport: we duplicate it off-screen with auto height
  // and overflow:visible so scrollHeight reports the true content size.
  var candidates = [];
  if (content) {
    candidates.push(content.offsetHeight);
    candidates.push(content.scrollHeight);
    try {
      var clone = content.cloneNode(true);
      clone.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:' +
        content.offsetWidth + 'px;height:auto;overflow:visible;visibility:hidden;pointer-events:none;';
      document.body.appendChild(clone);
      candidates.push(clone.scrollHeight);
      document.body.removeChild(clone);
    } catch (e) { /* ignore */ }
  }
  candidates.push(document.body.scrollHeight);
  candidates.push(document.documentElement.scrollHeight);
  var h = 0;
  for (var i = 0; i < candidates.length; i++) {
    if (typeof candidates[i] === 'number' && candidates[i] > h) h = candidates[i];
  }
